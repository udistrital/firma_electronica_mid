...
env = _get_env("RUN_MODE", "dev").lower()
API_PORT = _get_env("API_PORT", "8080")
DOCUMENTOS_CRUD_URL = _get_env("DOCUMENTOS_CRUD_URL")
GESTOR_DOCUMENTAL_URL = _get_env("GESTOR_DOCUMENTAL_URL")
VERIFICACION = _get_env("VERIFICACION")
VERIFICACION_EXTERNA = _get_env("VERIFICACION_EXTERNA")
QR_SIGNING_SECRET = _get_env("QR_SIGNING_SECRET")
...
def checkEnv():
    for variable in variables:
        if variable not in os.environ:
            print(f"{variable} environment variable not found")
            sys.exit()

    if "QR_SECRET_NAME" not in os.environ:
        print("QR_SECRET_NAME environment variable not found")
        sys.exit()


def get_api_port():
    return _get_env("API_PORT", "8080")


def get_documentos_crud_url():
    return _get_env("DOCUMENTOS_CRUD_URL")


def get_gestor_documental_url():
    return _get_env("GESTOR_DOCUMENTAL_URL")


def get_verificacion_url():
    return _get_env("VERIFICACION")


def get_verificacion_externa_url():
    return _get_env("VERIFICACION_EXTERNA")
...
def get_qr_signing_secret():
    return _get_env("QR_SIGNING_SECRET")
